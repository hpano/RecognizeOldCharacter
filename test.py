...
def set_data(targets, target_name, num_train, filter, img_size):
    ...
    for i in range(memory_size, load_size):
        ...
        print(one_hot)  # for debug

        # 要修正
        code_reliability[1][codes[0]][codes[1]] += 1
        code_reliability[0][codes[1]][codes[0]] += 1
        code_reliability[1][codes[1]][codes[2]] += 1
        code_reliability[0][codes[2]][codes[1]] += 1
...
(x_train, t_train) = set_data(traindata, "train", num_train, filter, img_size)
